...
def is_file_present(file_path, reorganisation=None):
    ...
    if os.path.exists(file_path):
        ...
        with open(err_file_path, 'r') as err_file:
            ...
            if any(word in error_content for word in ['OptError', 'failed', 'Could not converge SCF', 'ValueError', 'Could not converge geometry optimization', 'Please restart from the most recent geometry', 'DUE TO TIME LIMIT', 'CANCELLED AT', 'optking.exceptions.AlgError:']):
                return 'Calculation failed'
            else:
                return 'Waiting'
